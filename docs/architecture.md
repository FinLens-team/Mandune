...
flowchart TB
  ...
  Evidence --> Tencent[Tencent public market source]
...
| `src/extraction/` | Screenshot extraction boundary; the current UI does not enable it |
| `deploy/` | Reproducible release, Nginx/systemd installation, rollback, and purge jobs |
...
2. The user confirms a draft containing holdings, observation dates, and four personal constraints.
3. The server validates the draft and freezes it as a versioned portfolio snapshot.
4. The selected executor collects evidence and computes deterministic portfolio derivations.
5. The model receives only the bounded analysis input assembled by the prompt compiler.
6. Task events and optional text deltas reach the client while the analysis runs.
7. The server validates the complete result before marking it ready and writing immutable history.
8. Atlas generation runs as a non-blocking follow-up; failure cannot rewrite a completed review.
...
`MANDONG_ANALYSIS_MODE=stream` is the default when a model is configured. It uses the unauthenticated Tencent market source and one streaming model generation. Headings from the partial Markdown can drive progress, but the report is published only after the full text passes validation.
