...
This document defines observable product behavior. It does not select an exact framework, model provider, market-data vendor, authentication protocol, or deployment API.
...
- One prebuilt portfolio snapshot containing a small, fixed set of simulated holdings.
...
Phase 0 does not prove real-user onboarding, screenshot extraction quality, account persistence, or broad asset coverage.
...
The first session asks the user to choose a presentation theme or accept a clear default. The choice may change vocabulary, artwork, pacing, and the narrative front, but it must not change calculations, evidence selection, confidence, risk classification, or directional guidance for identical inputs.
...
The adapter must not manufacture a value for an unsupported symbol, confuse retrieval time with market time, or collapse stale and current data into the same status. The product can replace adapters without changing the portfolio, evidence, or card contracts.
...
The MVP does not issue or execute orders, prescribe an exact trade quantity as a personalized instruction, guarantee outcomes, claim to beat the market, or present the result as regulated fiduciary, tax, or legal advice. The user remains the decision-maker. High-impact or weakly supported cases route to review rather than stronger language.

### 5.12 Limited Analysis and Degradation

Analysis coverage is visible as a status, not hidden in generic confidence wording. FinLens returns a limited analysis when at least one material input is unknown, ambiguous, stale beyond the product's disclosed policy, unsupported, contradictory, or unavailable.
...
| Analysis generation times out or returns malformed content | No partial text is presented as a completed recommendation | Retry safely without duplicating holdings or snapshots |
...
- The market-data adapter returns timestamps and typed failure/staleness states. Contract tests demonstrate available, stale, ambiguous, unsupported, rate-limited, and failed outcomes without binding the analysis layer to one vendor.
