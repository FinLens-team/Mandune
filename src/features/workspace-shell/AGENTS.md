...
- Data management has no cancel action; saving returns to home, while cross-page navigation for data management, history, atlas, theme switching, and about goes through the persistent drawer trigger.
...
- The home view is a dark poster stage scoped by `--poster-*` custom properties on `.workspace-home`; the portfolio view keeps the global light tokens. The poster is deliberately minimal: no visible title block and no badges; the only heading is a visually-hidden h1 (`.workspace-home__sr-only`, 满懂 · 每日复盘) that keeps the section label and focus management working. A top bar carries the small white brand mark plus a「主页」page label so users can tell where they are. The mascot is the single analysis entry (`nailong-laugh.webp` inside one button); there is deliberately no CTA button — the hint text lives under the mascot (see the dialog bullet), and the confirm dialog provides the second-step confirmation. Poster copy comes from real journey state plus the「我是龙」theme strings — no return figures or forecasts may be added to captions because draft lines carry no observed performance field.
- The background `workspace-home__ticker` SVG is symbolic decoration only (three drifting sparkline loops, gold stroke, low stroke-opacity); it must never encode or resemble actual market data, and reduced motion freezes it in place via the shared kill-switch blocks. Each path holds two copies of an 800-unit pattern so a full ±800px slide loops seamlessly; line subtlety lives in `stroke-opacity` (not element `opacity`) because the reduce-motion block forces `opacity: 1 !important` on poster children. (A candlestick-cluster variant was tried and rejected by the user — the polylines are the accepted look.)
- Theme captions render as bilibili-style danmaku (`workspace-home__danmaku`): each theme's `copy.danmaku` in `src/theme/catalog.ts` is the single extension point. Spawns pick a random caption (no on-screen duplicates) and a lane from the sky/low bands only (`DANMAKU_LANES`, never across the centered mascot), fly right → left at constant linear speed behind the hero layer (`z-index: 1`), cap at 8 concurrent, pause when the page is hidden, and the layer is removed entirely under reduced motion.
...
- `AnalysisConfirmDialog` is deliberately minimal by user decision: only the e2e-bound heading「按当前输入发起今日复盘？」plus 取消/开始复盘 actions on the dark poster surface — no constraint grids, runtime facts, or disclosure blocks. The natural-language action hint (进行今日复盘/复盘进行中) is plain gold text under the mascot inside the hero button, never a styled button.
