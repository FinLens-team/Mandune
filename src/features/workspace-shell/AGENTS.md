...
- The background `workspace-home__ticker` SVG is symbolic decoration only: two rows of gold candle clusters (`TICKER_CANDLES_HIGH/LOW` data + `TickerCandles` renderer in WorkspaceShell.tsx) drifting in opposite directions. It must never encode or resemble actual market data, and reduced motion freezes it in place via the shared kill-switch blocks. Each row is duplicated at +800 so a full ±800px slide loops seamlessly; subtlety lives in `stroke-opacity`/`fill-opacity` (not element `opacity`) because the reduce-motion block forces `opacity: 1 !important` on poster children.
