...
- The home view is a dark poster stage scoped by `--poster-*` custom properties on `.workspace-home`; the portfolio view keeps the global light tokens. The poster hierarchy: a small gold theme signature「哈呃呃涨涨」on top (expression only), the runtime-state title (今日持仓观察/复盘进行中) as the only display headline, then the hero. The hero is one button combining the centered mascot (`nailong-laugh.webp`) and a persistent gold CTA pill (点奶龙，开始今日复盘/复盘进行中，点奶龙继续) so the primary action is always readable, not hidden inside an image. Poster copy comes from real journey state plus the「我是龙」theme strings — no return figures or forecasts may be added to badges or captions because draft lines carry no observed performance field; the「随机体验数据/体验持仓已编辑」badge is a product invariant and must stay.
...
- The single caption is pinned directly above the mascot (`inset: auto 0 100%` on the stage), cycles the theme strings every 4800ms (`caption-fade` matches `HOME_CAPTION_CYCLE_MS`), is text-only (the pill shape is reserved for real data), pauses when the page is hidden, and is removed entirely under reduced motion.
- The home poster no longer carries the brand logo; the white-on-transparent logo remains in the drawer footer's dark chip and must never sit on a light surface. The home view is locked to `height: 100dvh` with `overflow: clip`; the hero scales via `max-height: 100%` instead of scrolling, and the error message floats absolutely inside the poster so it is never clipped away.
