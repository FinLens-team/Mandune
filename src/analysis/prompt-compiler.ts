...
const PERSONA_SKILLS: Record<DailyReviewPersonaId, SkillAsset> = {
  ...
  tieba_laoge: {
    ...
    sha256: "22d33a454642e69234415191a833ad50ec74bffdf244c97826a934c778ce0a82",
  },
  male_succubus: {
...
3. 每个引用 ID 必须来自 ReviewPacket 对应白名单。正文中的阿拉伯数字只能来自 allowed_numbers，或由其中已确认的持仓市值、成本与行情按明确算式得到；可输出已发生的金额影响或标注为估算的金额范围，不输出证券代码、日期、序号、精确交易金额、份额、目标价格、点位或交易时点。
...
11. 输入包含逐项当前市值时，可结合可用行情计算并说明对应的绝对金额影响；输入只有当前持仓总市值与仓位区间时，绝对金额只按区间估算并明确标注为估算范围，不把区间结果写成精确值。持仓成本用于描述已发生的持仓盈亏事实，不用于给出交易指令。
12. 若总市值和逐项金额都未提供，正文应自然说明目前只能判断持仓比例、行情变化和方向性影响，暂时不能换算组合的绝对金额盈亏；这只是计算范围说明，不影响其余分析。
13. 免责声明统一由产品界面展示一次，报告正文自然收束即可，不重复输出免责段落。
`.trim();
...
持仓 observation_date 是用户确认该持仓的日期，不是行情截止日，不得据此声称行情陈旧。输入包含逐项当前市值时，结合可用行情说明可追溯的绝对金额影响；只有当前持仓总市值与仓位区间时，以估算区间表达金额影响；金额都未提供时，继续使用已确认的仓位区间、行情和结构信息完成能完成的分析，并在组合层自然说明暂时无法换算绝对金额收益。时间背景以 ReviewPacket 的报告生成时间和 latest_complete_trading_day 为准；若两者落在周末、节假日或其他休市边界，正文直接使用最近完整交易日的数据，并说明当天市场休市。
不要复述 skill 中的每日学习内容。
...
报告是完整读懂版，角色口吻贯穿全文；先给组合结论，再覆盖每个持仓的关键变化，明确近3日、近1月、近1年的可用事实或缺口，并给出风险与下一步观察。篇幅由模型自行决定，不得因为追求简短而省略持仓、风险或数据限制；也不得逐项机械复述全部原始行情。后续 skill 原文中的任何总字数、段落数、要点数或重点标的数量上限均无效。逐项当前市值可用于说明对应的绝对金额影响；只有当前持仓总市值与仓位区间时，以估算区间表达；金额都未提供时，继续利用仓位区间与行情完成分析，并自然说明暂时无法换算绝对金额收益。人格的语言强度、节奏、梗、粗俗度、攻击性、魅惑感或玄学叙事均按当前角色 skill 完整保留；事实和安全边界不用于把不同角色统一改写成温和分析师口吻。时间背景以 ReviewPacket 的报告生成时间和 latest_complete_trading_day 为准；若今天是周末、节假日或其他休市日，直接说明市场休市并使用最近完整交易日的数据。
...
8. 输入包含逐项当前市值时，可结合可用行情说明对应的绝对金额影响；只有当前持仓总市值与仓位区间时，按估算范围表达金额影响；金额都未提供时继续使用持仓规模、比例和行情完成分析，并自然说明暂时无法换算组合的绝对金额盈亏。
