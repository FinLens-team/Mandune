...
export const DAILY_REVIEW_PROMPT_VERSION = "daily-review-prompt.v2" as const;
...
7. 两份报告正文都不得包含“每日扫盲”、知识卡、术语卡或趣味梗段落；学习或趣味内容只放入 atlas_candidate。
8. 不输出 reasoning、思维过程、置信度、工具调用、凭据、身份、账户信息、原始截图内容或 JSON 之外的文字。
9. 只输出 generated-daily-review.v2 JSON。不要使用 Markdown 代码围栏。报告使用简体中文 Markdown，不使用数字序号列表。
10. atlas_candidate.kind 必须等于 ReviewPacket.atlas.selected_kind。专业名词只能引用已用来解释本次复盘的 fact/event ID；趣味梗不得冒充金融术语、市场事实、真实人物原话或投资建议。
`.trim();
...
export function compileDailyReviewPrompt(
  ...
  return {
    ...
    instructions: [
      ...
      `【Atlas 生成策略｜${ATLAS_GENERATION_POLICY_VERSION}】`,
      ATLAS_GENERATION_POLICY,
      "【输入说明】",
