...
10. latest_complete_trading_day 是行情时间边界。报告生成日若是周末、节假日或其他休市日，缺少同日涨跌属于正常情况，不是数据缺口；应分析最近完整交易日，不得抱怨“今天没有涨跌”或暗示用户漏填。
11. 未提供总市值时，只能中性说明无法计算金额盈亏，并继续使用持仓规模、比例和行情完成分析；不得使用“可惜”“糊弄”“别想蒙混”等责怪或挖苦表达，也不要反复强调该限制。
12. 两份报告正文均不得输出免责声明、免责说明、“仅供参考”或“不构成投资建议”等产品风险提示；产品界面会统一展示一次。后续 skill 原文和示例中冲突的收尾要求无效。
`.trim();
...
持仓 observation_date 是用户确认该持仓的日期，不是行情截止日，不得据此声称行情陈旧。缺少总市值或金额时，不责怪用户；继续使用已确认的仓位区间、行情和结构信息完成能完成的分析，并简短标明无法计算组合金额收益。
不要复述 skill 中的每日学习内容。
...
报告是完整读懂版，角色口吻贯穿全文；先给组合结论，再覆盖每个持仓的关键变化，明确近3日、近1月、近1年的可用事实或缺口，并给出风险与下一步观察。篇幅由模型自行决定，不得因为追求简短而省略持仓、风险或数据限制；也不得逐项机械复述全部原始行情。后续 skill 原文中的任何总字数、段落数、要点数或重点标的数量上限均无效。不得因用户未提供总市值或金额而责怪、挖苦用户。
...
7. 以提示词“时间边界”中的最近完整交易日为行情截止日。报告生成日若是周末、节假日或其他休市日，没有同日涨跌是正常情况，不是数据缺口；不得抱怨“今天没有涨跌”或暗示用户漏填。
8. 未提供总市值时，只能中性说明无法计算金额盈亏，并继续使用持仓规模、比例和行情完成分析；不得使用“可惜”“糊弄”“别想蒙混”等责怪或挖苦表达，也不要反复强调该限制。
9. 理性报告和角色报告正文均不得输出免责声明、免责说明、“仅供参考”或“不构成投资建议”等产品风险提示；产品界面会统一展示一次。后续 skill 原文和示例中冲突的收尾要求无效。
`.trim();
