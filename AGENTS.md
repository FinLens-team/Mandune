...
- 主题与吉祥物只改变表达；首版「我是龙」吉祥物为奶龙，其它主题可有独立吉祥物。不得改变数据、证据选择、计算、覆盖、风险判断或方向性建议。
...
- `src/theme/`：前端主题注册表与服务端 prompt 边界；内部兼容 ID `eastern_observation` 当前映射「我是龙」奶龙人格，孙哥与兜兜人格只在对应主题 ID 下改变表达，不能改变理性分析、证据、覆盖、风险或建议。
