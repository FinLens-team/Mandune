...
| 盘前日报 | 每日预生成七主题公开市场简报，包含三大指数及最多四条可核验市场/宏观新闻，明确日报日期、行情截止、发布时间和原文来源 |
...
等待页日报与持仓复盘是两条独立链路。日报每天预生成七个主题版本，只使用公开的三大指数和经过时间、域名、相关性、隐私与引用校验的市场/宏观新闻，不读取工作区或持仓；没有合格新闻或 AKShare 新闻接口失败时，`news` 显式为空，模型不得补写。详情见 [日报流水线](docs/daily-briefing-pipeline.md)。
...
- 扩展更多证据与新闻来源适配器，并继续保持供应商故障、弱相关条目和引用异常时的显式降级；
